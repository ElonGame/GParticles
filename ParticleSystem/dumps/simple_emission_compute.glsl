...
layout(binding = 0, offset = 0) uniform atomic_uint simple_aliveParticles;
layout(binding = 1, offset = 0) uniform atomic_uint simple_emissionAttempts;
layout(binding = 2, offset = 0) uniform atomic_uint randomCounter;

layout(std430, binding = 3) buffer Simple_lifetimes
{
	float simple_lifetimes[];
};

layout(std430, binding = 4) buffer Simple_lastPositions
{
	vec4 simple_lastPositions[];
};

layout(std430, binding = 5) buffer Simple_lineLifetime
{
	float simple_lineLifetime[];
};

layout(std430, binding = 6) buffer Simple_size
{
	float simple_size[];
};

layout(std430, binding = 7) buffer Simple_positions
{
	vec4 simple_positions[];
};

layout(std430, binding = 8) buffer Simple_velocities
{
	vec4 simple_velocities[];
};

layout(std430, binding = 9) buffer Simple_colors
{
	vec4 simple_colors[];
};

layout(std430, binding = 10) buffer Simple_texCoords
{
	vec2 simple_texCoords[];
};
...
void emission()
{
	simple_lifetimes[gid] = 5;
	simple_positions[gid] = spherePositionGenerator(1.5, false);
	simple_velocities[gid].xyz = velocityGenerator(simple_positions[gid].xyz, vec3(0), 0.1);
	simple_colors[gid] = vec4(0.1,0.9,0.1,1);
}
...
void main()
{
	...
	if (atomicCounter(simple_aliveParticles) >= simple_maxParticles)
		return;

	// if particle is not currently alive at this gid another can be emitted
	if (simple_lifetimes[gid] <= 0)
	{
		// if max number of particles toCreate on current iteration has not
		// been reached emitt a new particle
		if (atomicCounterIncrement(simple_emissionAttempts) < simple_toCreate)
		{
			atomicCounterIncrement(simple_aliveParticles);

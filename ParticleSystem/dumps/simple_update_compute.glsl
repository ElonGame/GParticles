...
void update()
{
	simple_positions[gid].xyz += simple_velocities[gid].xyz * simple_deltaTime;
}
...
void main()
{
	...
	// age particle
	simple_lifetimes[gid] -= simple_deltaTime;

	// if particle just died
	if (simple_lifetimes[gid] <= 0)
	{
		simple_lifetimes[gid] = -1;
		atomicCounterDecrement(simple_aliveParticles);

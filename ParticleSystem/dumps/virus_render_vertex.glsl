...
in vec4 virus_velocities;
in float virus_floorHit;
in vec4 virus_lastPositions;
in float virus_lifetimes;
in float virus_size;
in float virus_lineLifetime;
in vec4 virus_positions;
in vec4 virus_colors;
in vec2 virus_texCoords;
in vec4 virusPos;

out vec4 virus_velocitiesV;
out float virus_floorHitV;
out vec4 virus_lastPositionsV;
out float virus_lifetimesV;
out float virus_sizeV;
out float virus_lineLifetimeV;
out vec4 virus_positionsV;
out vec4 virus_colorsV;
out vec2 virus_texCoordsV;
out vec4 virusPosV;
...
//if (virus_size==0.025) rot = 10;
	...
	mat4 rotation = rotationMatrix(vec3(1,1,0.5), radians(rot * virusAnimationAngle));

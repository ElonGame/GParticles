...
layout(binding = 0, offset = 0) uniform atomic_uint randomCounter;
...
layout(std430, binding = 3) buffer Puddle_floorHit
{
	float puddle_floorHit[];
};

layout(std430, binding = 4) buffer Puddle_lastPositions
{
	vec4 puddle_lastPositions[];
};

layout(std430, binding = 5) buffer Puddle_lifetimes
{
	float puddle_lifetimes[];
};

layout(std430, binding = 6) buffer Puddle_size
{
	float puddle_size[];
};

layout(std430, binding = 7) buffer Puddle_lineLifetime
{
	float puddle_lineLifetime[];
};

layout(std430, binding = 8) buffer Puddle_colors
{
	vec4 puddle_colors[];
};
...
layout(std430, binding = 10) buffer Puddle_texCoords
{
	vec2 puddle_texCoords[];
};

layout(std430, binding = 11) buffer Puddle_velocities
{
